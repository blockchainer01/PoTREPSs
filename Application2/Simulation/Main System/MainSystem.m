...
%% first: define variables needed for optimization
for i=1:Ns
    x_hat{1,i}=zeros(4,tmax+1);
    u_hat{1,i}=zeros(1,tmax);
    u_att{1,i}=zeros(1,tmax);
    x_til{1,i}=zeros(4,tmax+N);
    for j=1:Ns
        if A_comm(i,j)>0
            % attacked information
            x_til_att{i,j}=zeros(4,tmax+N);
        end
    end
    ...
    for k=1:N
        x_til{1,i}(:,k)=(A{i,i}+B{i,i}*K{i,i})^(k-1)*x{1,i}(:,1);
    end
end
...
for t=1:tmax
    ...
    for i=1:Ns
        ...
        for j=1:Ns
            if A_comm(i,j)>0
                ...
                if attack_flag_x{i,j}(t)==0
                    for k=1:N
                        x_til_att{i,j}(:,t+k-1)=x_til{1,j}(:,t+k-1);
                    end
                ...
                % else being attacked
                    ...
                    if t==1
                        for k=1:N
                            x_til_att{i,j}(:,k)=x_til{1,j}(:,k)+Ay*sin(Omega*(t));
                        end
                        continue_attack_x{i,j}(t)=1;
                    else
                        for k=1:N
                        x_til_att{i,j}(:,t+k-1)=x_til{1,j}(:,t+k-1)+Ay*sin(Omega*(t));
                        end
                ...
                for k=1:N
                    ...
                    for j=1:Ns
                        if A_comm(i,j)>0
                            ...
                            sum_xj(:,k)=sum_xj(:,k)+A{i,j}*x_til_pred{i,j}(:,k);
                        end
                    end
                    Con=[Con;
                        x_pred(:,k+1) == A{i,i}*x_pred(:,k)+B{i,i}*u_pred(k)+sum_xj(:,k)];
                end
                % 2. about H
                Con=[Con;
            ...
            if system_action_u{1,i}(node,t)==1
                ...
                for k=1:N
                    ...
                    for kk=1:size(x_pred_node{1,node},1)
                        x_pred_veri=A{i,i}(kk,:)*x_pred_node{1,node}(:,k)...
                            +B{i,i}(kk,:)*u_pred_node{1,node}(k)+sum_xj(kk,k);
                ...
                if size(find(Con_veri==0),1)~=0
                    ...
                    if t==1
                        score{1,i}(node,t)=score0^2;
                    else
                        score{1,i}(node,t)=(score{1,i}(node,t-1))^2;
                    end
                else
                    % meet the conditions to participate in the comparison
                    if t==1
                        score{1,i}(node,t)=score0;
                    else
        ...
        for k=1:Steps_con
            ...
            u{1,i}(t+k-1)=u{1,i}(t);
            u_att{1,i}(t+k-1)=u_att{1,i}(t);
            x_hat{1,i}(:,t+k-1)=x_pred_best(:,1);
            score{1,i}(:,t+k-1)=score{1,i}(:,t);
        end
        if t==1
        ...
        x{1,i}(:,t+1) = A{i,i}*x{1,i}(:,t)+B{i,i}*u_att{1,i}(t)+sum_xj+AD{i,i}*w(:,i,t);
        % itetime=toc;
        ACE(i,t+1)=C{i,i}*x{1,i}(:,t+1);
        df(i,t+1)=x{1,i}(1,t+1);
        dP(i,t+1)=x{1,i}(4,t+1);
    end
    ...
    if mod(t,Steps_fig)==1
        f = figure(1);
        subplot(2,1,1),plot(df(:,1:t)'),axis([0,tmax,-0.6,0.6]),legend('df1','df2','df3');
        xlabel('step'),ylabel('df'),title('State Responses');
        subplot(2,1,2),plot(dP(:,1:t)'),axis([0,tmax,-3,3]),legend('dP1','dP2','dP3');
        xlabel('step'),ylabel('dP'),title('State Responses');
        drawnow
        F=getframe(gcf);
        img=frame2im(F);
        [img,map]=rgb2ind(img,256);
        if pic_num == 1
            imwrite(img,map,'StateResponses.gif','gif','Loopcount',inf,'DelayTime',0.5);
        else
            imwrite(img,map,'StateResponses.gif','gif','WriteMode','append','DelayTime',0.5);
        end
        pic_num = pic_num + 1;
    end
end
run_time=toc;
save 'RunResult.mat' x x_hat x_til x_til_att u u_att df dP ACE system_action_u state_u...
    Ay Au Omega attack_flag_u attack_flag_x tmax run_time