...
'''
Physical Layer of the Microgrid(PLM) node code 

PLM receives the control signal u (ts) from DRL and sends it to the device. 
After collecting the status signal x (ts), the device sends it to other PLM 
nodes and also receives x (ts) signals from other nodes. After running, it 
obtains x (ts+1) signals and sends them to each node of DRL.

The simulation experiment directly uses the method to run the state space 
equation at the node, obtaining x (ts+1)
...
THIRD_LAYER_IP = local_ip  # 示例中的端口，每个第三层节点需要不同的端口  
THIRD_LAYER_PORT = local_port # 示例中的端口，每个第三层节点需要不同的端口  
...
def modezz(arr):  
    ...
    most_common_rows = arr[np.where(counts == max_count)[0][0]]  # 这里只取第一个出现次数最多的行，如果有多个，需要额外处理   
    ...
    try:
        ...
        if data:
            ...
            if addr[0][10] == '7':
                ...
                pass
            #print(f"u_flag={u_flag},x_flag={x_flag}")
